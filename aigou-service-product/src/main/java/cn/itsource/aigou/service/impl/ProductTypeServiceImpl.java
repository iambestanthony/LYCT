...
public class ProductTypeServiceImpl extends BaseServiceImpl<ProductType> implements IProductTypeService {
	...
	public List<ProductType> getAllTree(Long pid) {
		...
		//return getAllProductTypesByRes(pid);
	}
	...
	public List<ProductType> getAllProductTypesByRes(Long id) {
		List<ProductType> children = mapper.getChildren(id);
		if (children== null || children.size() == 0) {
			return null;
		}
		for (ProductType productType : children) {
			List<ProductType> afterChildren = getAllProductTypesByRes(productType.getId());
